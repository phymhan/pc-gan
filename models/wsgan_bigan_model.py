...
class WSGANBiGANModel(BaseModel):
    def name(self):
        return 'WSGANBiGANModel'
    ...
    def initialize(self, opt):
        ...
        self.loss_names = ['G_GAN', 'E_GAN', 'G_IP', 'cycle_x', 'cycle_y', 'D_real', 'D_fake']
    ...
    def backward_D(self):
        # (G(y), y) = (fake_x, real_y)
        pred_fake = self.netD(self.fake_x.detach(), self.real_y)
        self.loss_D_fake = self.criterionGAN(pred_fake, False)

        # (x, E(x)) = (real_x, fake_y)
        pred_real = self.netD(self.real_x, self.fake_y.detach())
    ...
    def backward_GE(self):
        # (G(y), y) = (fake_x, real_y)
        pred_fake = self.netD(self.fake_x, self.real_y)
        self.loss_G_GAN = self.criterionGAN(pred_fake, True)

        # (x, E(x)) = (real_x, fake_y)
        pred_real = self.netD(self.real_x, self.fake_y)
        self.loss_E_GAN = self.criterionGAN(pred_real, False)
        ...
        # Combined loss
        self.loss_GE = self.loss_G_GAN + self.loss_E_GAN + self.loss_G_IP + self.loss_cycle_x + self.loss_cycle_y
