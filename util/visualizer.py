...
class Visualizer():
    def __init__(self, opt):
        ...
        self.save_all_images = opt.save_all_images
        self.save_html_freq = opt.update_html_freq
        self.num_iter_per_epoch = opt.num_iter_per_epoch
    ...
    def display_current_results(self, visuals, epoch, iteration, save_result):
        ...
        if self.use_html and (save_result or not self.saved):  # save images to a html file
            ...
            if self.save_all_images:
                for label, image in visuals.items():
                    image_numpy = util.tensor2im(image)
                    img_path = os.path.join(self.img_dir, 'epoch%.3d_%.6d_%s.png' % (epoch, iteration, label))
            ...
            if self.save_all_images:
                for it in range(iteration, 0, -self.save_html_freq):
                    ep = math.ceil(iteration / self.num_iter_per_epoch)
                    webpage.add_header('epoch [%d], iteration [%d]' % (ep, it))
                    ims, txts, links = [], [], []
                    for label, image in visuals.items():
                        # image_numpy = util.tensor2im(image)
                        img_path = 'epoch%.3d_%.6d_%s.png' % (ep, it, label)
                        ims.append(img_path)
                        txts.append(label)
