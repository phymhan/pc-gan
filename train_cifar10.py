...
class Options():
    def initialize(self, parser):
        ...
        parser.add_argument('--fineSize', type=int, default=32, help='scale images to this size')
...
def get_model(opt):
    ...
    if opt.which_model == 'alexnet':
        ...
        net = networks.ResNet(3, opt.num_classes, opt.which_model, pretrained=False)
...
if __name__=='__main__':
    ...
    if opt.mode == 'train':
        ...
        transform = transforms.Compose([transforms.Resize(opt.fineSize), transforms.ToTensor(), transforms.Normalize((0.5, 0.5, 0.5), (0.5, 0.5, 0.5))])

        trainset = torchvision.datasets.CIFAR10(root=opt.dataroot, train=True,
        ...
        opt.dataset_size = len(trainset)
        opt.dataset_size_val = len(testset)
        print('dataset size = %d' % len(trainset))
        # train
        train(opt, net, trainloader, testloader)
